...
if(prizeData.id < 4) {
    return (
        <div className="flex flex-col items-center justify-center border-card_dark border-4 rounded-xl p-2 bg-white sm:w-full sm:mx-6 sm:p-1 h-full font-primary font-extrabold">
            <div className="flex flex-col border-x-card_light border-card_dark border-4 py-20 px-4 sm:p-1 h-full rounded-xl w-full">
              ...
              <h3 className="text-center text-card_text text-2xl m-auto my-8 w-70">{prizeData.name}</h3>
                  {prizeData.prizeDesc.map((incentive)=> {
                      return(
                          <li key={incentive} className="w-full m-auto list-disc text-card_text">{incentive}</li>
                      )
                  })}
            </div>
        </div>

  ...
  return (
    <footer className="flex items-center justify-center bg-footer_clr rounded-xl m-8 text-primary_dark px-14 py-6 md:flex-col md:px-8 font-primary">
        ...
        {footerMenu.map((item) => {
          return (
            <Link href={item.url} key={item.name}>
              <a className="text-left py-2 text-xl font-extralight midLg:text-center">
                {item.name}
              </a>
            </Link>
          );
      ...
      <div>
        ...
        <h3 className="flex flex-col text-center justify-center text-primary_dark">
          Copyright Ⓒ {new Date().getFullYear()} CrossKnot Community. All rights
          reserved.
        </h3>
      </div>
      <div className="flex flex-col">
        <div className="flex lg:flex-col">
          <div className="flex flex-col items-center justify-center">
            <h3 className="text-center font-bold font-primary text-2xl mb-4">
              Find us Online
            </h3>
              ...
              {socials.map((social) => {
                return (
                  <Link href={social.url} key={social.name}>
                    <a
                      className="hover:scale-110 transition-all duration-1000 ease-linear"
                      target="_blank"
                    >
